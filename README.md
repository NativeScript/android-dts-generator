...
Generate definitions following any of the approaches described below. Once you have run any of the commands, you can find the generated typings in `dts-generator/out` directory.
## Generate definitions for Android SDK
```shell
cd dts-generator
./gradlew jar
...
In order to test the tool there are some predefined AndroidX packages. We are running the tool and comparing the output with the expected output committed in the repo.
To run the test:
...
Then check whether the make command returns an error when comparing the files.

> Note: If the contents of the libs folder are changed, the expected output should also be changed
...
There are different android support versions and they depend on main android classes it is a little bit complicated to generate those typings. One option is to generate a big **d.ts** file with all the libraries inside. The downside of this approach is that you have to generate a big file for every API level and if the android support version is changed all those **d.ts** files need to be regenerated.
To avoid this there is some functionality in the tool for passing dependencies when generating typings.
...
2. Input generics - When trying to get the type of a parameter which is a generic class we cannot really get the generic types of that class, so we cannot generate working typings. To fix this we are adding information about the generics of each package at the end of the file with comments starting with `//Generics information:`.
  So to fix this we need to provide a file with all the generic information for the packages the current jar relies on. You need to create a file and copy all the generic information of the related packages and provide it in the **input-generics** argument. This will make all the generic classes referenced without passing types to pass **any** so that the output will be valid.

## Adding all implements for generic types
There is an option **all-generic-implements** which is disabled by default which controls whether to add implements for all interfaces (if they are more than one) to generic type declarations. The problem is that in most of the cases one of those interfaces is actually an extend, so it should be manually reviewed and fixed after generation.

## Finding package dependencies
If you want to generate typings for a package but you are not sure how you can get all the needed dependencies you can follow the steps below:
...
5. The command above will get the needed jar files for your dependency and will output them in the [dts-generator/jar-files](dts-generator/jar-files) folder (or you can optionally pass another output folder `-PjarsOutput=another-folder`)
...
1. Run the typings generator for androidx passing **android-17/android.jar** as a super jar
...
- AndroidX 17 typings(built with super jar from android API 17) can be reused until android API 22
- AndroidX 23 typings(built with super jar from android API 23) can be reused until android API 25
- AndroidX 26 typings(built with super jar from android API 26) can be reused for API 26 and 27
